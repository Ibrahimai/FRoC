...
void ILP_solve_3()
{
	try {
		...
		GRBVar* vars_2 = model.addVars(lb_vars_2, up_vars_2, NULL, type_vars_2, names_vars_2, num_of_paths);
		...
		for (int i = 0; i < paths.size(); i++)
		{
			...
			model.addGenConstrAnd(vars_2[i - 1], temp_constr, paths[i].size() - 2);
			delete[] temp_constr;
		}
		...
		for (int i = 0; i < num_of_paths; i++)
		{
			if (paths[i + 1][0].deleted)
				continue;
			model.addConstr(vars_1[i] + vars_2[i] <= 1);
		}
		...
		for (int i = 0; i < num_of_paths; i++)
		{
			if (vars_1[i].get(GRB_DoubleAttr_X) && vars_2[i].get(GRB_DoubleAttr_X)  < 0.99999)
			{
				if (delete_path(i + 1))
					total++;
			...
			else
			{
				if (vars_1[i].get(GRB_DoubleAttr_X) <= 0.9999)
					std::cout << i << " " << vars_1[i].get(GRB_DoubleAttr_X) << std::endl;
				assert(vars_1[i].get(GRB_DoubleAttr_X) > 0.9999);
			}
