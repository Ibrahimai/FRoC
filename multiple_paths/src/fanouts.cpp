...
void add_node_fanouts(routing_node & node, std::map<std::string, int> & branchLabel, 
    ...
    for(unsigned i = 0; i < node.children.size(); i++){
        if(branchLabel.find(node.children[i]->get_name()) != branchLabel.end()){
            ...
            if(node.children[i]->type != LCCOMB && node.children[i]->type != FF && node.children[i]->type != LOCAL_LINE && node.children[i]->type != LOCAL_INTERCONNECT) { //can't add LUTs / FFs to routing file
...
void add_fanouts_to_routing(routing_tree & tree, std::map<std::string, int> & branchLabel, std::ofstream & RoFile, std::string node_name){
    ...
    if(map_first_fanouts) {
        for (unsigned i = 0; i < node->children.size(); i++) {
            if (branchLabel.find(node->children[i]->get_name()) == branchLabel.end()) {
                if (node->children[i]->type != LCCOMB && node->children[i]->type != FF && node->children[i]->type != LOCAL_LINE && node->children[i]->type != LOCAL_INTERCONNECT) {
                    ostringstream fanout_name;
                    fanout_name << "FANOUT" << all_fanouts.size();
                ...
                else if (node->children[i]->type != FF){ //check to see if LUT/Local_Line/Local_Interconnect can be replicated
